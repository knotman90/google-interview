...
constexpr int LIM = 10e6 + 1;

inline void fill_Cs(const int n, int* C) {
  unordered_set<int> A;
  int M=-1;
  for (int i = 0; i < n; ++i) {
    int a;
    read(a);
    const auto f = A.find(a);
    if(f!=A.end())
      continue;
    else
      A.insert(a);
...
int main() {
  ...
  for (int i = LIM-1;i >= 2; --i) {
    if (C_As[i] && C_Bs[i]) {
      int sum=C_As[i] + C_Bs[i];
      int gc=gcds(C_As[i], C_Bs[i]);
      if ( gc!=1 && ((gc > best_gcd) || (gc ==  best_gcd && sum > best_sum))) {
        best_gcd = gc;
        best_sum = sum;
       // cout<<i<<" new best at "<<C_As[i]<<" "<<C_Bs[i]<<endl;
      }
    }
  }
  if (best_gcd == -1) best_sum = C_As[1] + C_Bs[1];
