...
int main() {
  ...
  for (int i = 1; i < k; i++) {
    int amodk = i;
    int complement = k - amodk;
    if (MODS.find(amodk) != MODS.end() && MODS.find(complement) != MODS.end()) {
      int val = complement;
      if (MODS[amodk] < MODS[complement]) val = amodk;
  ...
  if (MODS[0] > 0) rem += MODS[0] - 1;

  if (k % 2 == 0 && REM.find(k / 2) != REM.end()) rem -= 1;
