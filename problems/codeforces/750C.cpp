...
//what is the range of possible values for each contest?
//at first [-INF,+INF]
//We also know that when we are currently in division 1 our minimum value
//cannot be less than 1900 and by the same line of reasoning when in division
//2 our rating cannot be more than 1899
//so at each step we enforce those conditions
//the score obtained at each contest can be simply added to each part of the range
//if for any reason happen that the L > R then it means that there is an inconsistency in the given 
//data.
int main() {
