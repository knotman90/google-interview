...
template<class CONTAINER, class READ_Fn =reader<typename CONTAINER::value_type> >
...
int main() {
    ...
    sort(ALL(N));
    ll w1 = abs(a-N[0])+abs(N[0]-N[n-2])  ;
    ll w3 = abs(a-N[n-2]) + abs(N[0]-N[n-2]);

    ll w2 = abs(a-N[1])+abs(N[1]-N[n-1])  ;
    ll w4 = abs(a-N[n-1])+abs(N[1]-N[n-1])  ;
    cout<<min(min(w1,w3),min(w2,w4))<<endl;
    }
    return 0;
}
