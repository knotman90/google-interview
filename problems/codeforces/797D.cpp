...
void solve(const int node, int l, int r) {
  

  if(node < 0 || l > r)
    return;
  if (T[node][0] >= l && T[node][0] <= r)
    VALS.insert(T[node][0]);

  solve(T[node][1], l, std::min(T[node][0] - 1, r));

  solve(T[node][2], std::max(T[node][0] , l), r);
