...
int main() {
  ...
  string s, t;
  read(s, t);
  for (int start_t = 0; start_t <=t.size() - s.size(); start_t++) {
    count = 0;
    for (int index_t = start_t, index_s = 0; index_s < s.size();
         index_s++, index_t++) {
      if (s[index_s] != t[index_t]) pos[count++] = index_s+1;
    }
    if (count < min_mod) {
      min_mod = count;
      copy(pos.begin(), pos.begin() + count, Mpos.begin());
    }
  }

  cout << min_mod << "\n";
  for (int i = 0; i < min_mod; i++) cout << Mpos[i] << " ";
  cout << endl;
