...
{ std::cout << n; }
//reads multiple arguments
template<typename S, typename T, typename... types> inline void write(const S& sep, T &n, types &...args)
...
int main() {
    ...
    string s;    
    read(s);
    auto a0 = find_if(ALL(s),[](auto x){return x != 'a';});
    auto a1 = find_if(a0+1,s.end(),[](auto x){return x == 'a';});
    if(a0==s.end())
        a0=s.end()-1;
   
    for_each(a0,a1,[](auto& a){ a= (char)(mod((int)a-'a'-1,26)+'a'); });
    write(s);
