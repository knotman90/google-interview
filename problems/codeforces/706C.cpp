...
typedef std::pair<ull,ull> pullull;
...
#define B (LLONG_MAX);
   for(int i=n-2; i>=0 ; i--){
       ...
       g[1] = S[i] <= Sr[i+1];
       C[i].first = g[1]  ? mymin<ll>(C[i].first , ps) : C[i].first;

       g[2] = Sr[i] <= S[i+1];
       C[i].second = g[2] ? pf + N[i] : B;
       g[3] = Sr[i] <= Sr[i+1];
       C[i].second = g[3]  ? mymin<ll>(C[i].second,ps + N[i]) : C[i].second;
