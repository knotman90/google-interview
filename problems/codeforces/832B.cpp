...
int main() {
  ...
  string s;
  read(s);
  for (const char a : s) good[a] = true;

  string pattern;
  read(pattern);
  bool star = false;
  if (find(ALL(pattern), '*') != pattern.end()) star = true;
  ...
  while (n--) {
    ...
    int pidx = 0;
    int qidx = 0;
    for (; go && qidx < q.size(); pidx++) {
      if (pattern[pidx] != '?' && pattern[pidx] != '*')
        if (q[qidx++] != pattern[pidx]) {
          go = false;
          break;
        } else
          continue;
      //? cannot be substituted with a good one
      if (pattern[pidx] == '?')
        if (good.find(q[qidx++]) == good.end()) {
          go = false;
          break;
        } else
          continue;
      ...
      if (pattern[pidx] == '*') {
        int diff = q.size() - pattern.size() + 1;
        if (diff < 0) {
          go = false;
          break;
        }
        for (int j = 0; j < diff; j++) {
          if (good.find(q[qidx + j]) != good.end()) {
            go = false;
            break;
          }
        }
    ...
    if (go && pidx < pattern.size() - 1) go = false;
    go ? cout << "YES" : cout << "NO";
    cout << endl;
  }
